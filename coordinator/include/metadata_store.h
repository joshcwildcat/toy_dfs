...
public:
    ...
    std::deque<std::tuple<std::string, FileMeta, std::chrono::steady_clock::time_point>> deleted_entries_;  // Files pending chunk cleanup
    ...
    FileMeta* getFile(const std::string& path) {
        ...
        if (it == entries_.end() || !it->second.is_file) return nullptr;
...
private:
    ...
    void cleanupOldTombstones() {
        ...
        while (!deleted_entries_.empty()) {
            ...
            if (age <= TOMBSTONE_LIFETIME) break;
    ...
    void cleanupOldTombstonesInternal(std::function<void(const FileMeta&)> chunkDeleter) {
        auto now = std::chrono::steady_clock::now();
