...
std::unique_ptr<CoordinatorServiceImpl> CoordinatorServiceImpl::Create(const std::string& datanode_addr) {
    auto channel = grpc::CreateChannel(datanode_addr, grpc::InsecureChannelCredentials());
...
CoordinatorServiceImpl::CoordinatorServiceImpl(std::unique_ptr<DataNodeService::StubInterface> stub)
...
Status CoordinatorServiceImpl::PutFile(ServerContext* context, ServerReader<PutFileRequest>* reader,
        ...
        for (int i = 0; i < num_chunks; ++i) {
            ...
            std::unique_ptr<grpc::ClientWriterInterface<PutChunkRequest>> writer(datanode_stub_->PutChunk(&ctx, &chunk_resp));
            ...
            if (!writer->Write(chunk_req)) {
                ...
                std::cerr << "Failed to write chunk " << chunk_id << " to datanode: " << status.error_message() << " (code: " << status.error_code() << ")" << std::endl;
                return Status(grpc::StatusCode::INTERNAL, "Failed to write chunk: " + status.error_message());
            }
            writer->WritesDone();
            //std::cerr << "Finished sending chunk";
            Status status = writer->Finish();
            if (!status.ok()) {
                std::cerr << "Chunk " << chunk_id << " write finished with error: " << status.error_message() << " (code: " << status.error_code() << ")" << std::endl;
                return status;
            }
        }
...
Status CoordinatorServiceImpl::GetFile(ServerContext* context, const GetFileRequest* request,
        ...
        for (const auto& chunk : meta->chunks) {
            ...
            std::unique_ptr<grpc::ClientReaderInterface<GetChunkResponse>> reader(datanode_stub_->GetChunk(&ctx, chunk_req));
...
Status CoordinatorServiceImpl::LookupFile(ServerContext* context, const LookupFileRequest* req,
                      LookupFileResponse* resp) {
        FileMeta* meta = store_.getFile(req->path());
        if (!meta) return Status(grpc::StatusCode::NOT_FOUND, "File not found");
...
Status CoordinatorServiceImpl::DeleteFile(ServerContext* context, const DeleteFileRequest* req,
            ...
            for (const auto& chunk : meta.chunks) {
                ...
                if (!status.ok()) {
                    std::cerr << "gRPC error deleting chunk " << chunk.chunk_id
                              << " during tombstone cleanup: " << status.error_message() << std::endl;
                } else if (!del_resp.success()) {
                    std::cerr << "Failed to delete chunk " << chunk.chunk_id
                              << " during tombstone cleanup: " << del_resp.error_message() << std::endl;
                }
            }
        });
    }
