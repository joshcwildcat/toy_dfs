...
DFSClient::DFSClient(const std::string& address)
    : stub_(dfs::CoordinatorService::NewStub(grpc::CreateChannel(address, grpc::InsecureChannelCredentials()))) {}
...
void DFSClient::putFileSync(const std::string& filename) {
    ...
    std::unique_ptr<grpc::ClientWriter<dfs::PutFileRequest>> writer(stub_->PutFile(&ctx, &resp));
...
std::string DFSClient::getFileSync(const std::string& filename) {
    ...
    std::unique_ptr<grpc::ClientReader<dfs::GetFileResponse>> reader = stub_->GetFile(&ctx, req);
...
bool DFSClient::deleteFileSync(const std::string& filename) {
    ...
    if (status.ok() && resp.success()) {
        ...
        throw std::system_error(ENOENT, std::system_category(), "No such file or directory");
    } else {
        // Protocol/network error: include StatusCode in message
        std::string error_msg = "gRPC error [" +
